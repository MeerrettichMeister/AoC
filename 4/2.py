...
def bingo(board: list,bingo: list, num):
    marked = list(it.chain.from_iterable(bingo))
    for x in range(0,25):
        if marked[x] == -1:
            marked[x] = 0
    unbingo = [item for item in board if item not in marked]
    print(num)
    print(sum(unbingo)*num)
    print("Bingo")
    return 1

def update(board: BingoBoard, val):
    if val in board.board:
        board.checks[board.board.index(val)//5][board.board.index(val)%5] = val

def checkWin(board: BingoBoard, num):
    for column in board.checks:
        if -1 not in set(column):
            return bingo(board.board, board.checks, num)
...
def main():
    ...
    i = 0
    while i < len(field):
        bingoBoards.append(BingoBoard.createBoard(field[i:(i+25)]))
        i +=25
    round = 0
    winner = []
    for num in numbers:
        for boards in bingoBoards:
            ...
            if checkWin(boards,num) == 1:
                if boards not in winner:
                    winner.append(boards)
                    round = num
        if len(winner) == len(bingoBoards):
            break

    print(winner[-1])
    print(round)
    checkWin(winner[-1],round)
